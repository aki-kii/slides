...
<div style="view-transition-name: recommended-customize">
  <h2>おすすめのカスタマイズ: Snapshot Test</h2>
</div>
...
- リソースの設定を試行錯誤しているときに、CDKでプロパティの実装方法を調べるのは大変...
- CDKでリソースをデプロイしてみて、わからない設定があればマネコンから変更してみる
- 確定したタイミングでCDKを書き直せばセーブポイントになって安全
- これなら新しく提供されたサービスもすぐに検証できる！
- ドリフト検出でどんな設定したかもみれる（注意事項あり）

---

## ③ CDKの便利機能を活用するべし

CDKはAWSの設定を抽象化して直感的に扱いやすいようにしています

L2 Construct

- リソースを扱いやすい単位で設定できる(VPCとか)
- 複雑な仕組みを簡単に実装できるようになるメソッドもある(S3 AutoDeleteObjectとか)

Grants

- リソース間のIAMロールの設定を直感的かつめちゃくちゃ簡単に設定できる
- マネコンから設定するより遥かに簡単

fromLookup

- 既にデプロイされているリソースを参照可能に
- 既存のVPCとかRDSに依存した検証もできる
...
Constructとは？
1つ以上のAWSリソースをレゴブロックのように組み合わせるコンポーネント
〇〇のConstructにはこんな設定が含まれている

Custom Constructは自分でも作れる！
繰り返し使う構成や、理解しやすい構成でコンポーネント化して自分だけのConstructを作ろう！

技術検証する時に「よく使う」構成を育てることで、検証も素早くできるようになる

最近作ったCustom Construct
...
Coding AgentはCDKのソースコードをコンテキストとして扱えるため相性がいい！
変数やコメントも自然にコンテキストとして含められるので意図を汲み取ってもらいやすい(そのコードをオーバーレイ)

AWS Infrastructure as Code MCP
